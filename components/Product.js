...
function Product({ id, title, category, description, image, price, rating,isPrime  }) {

  const dispatch=useDispatch();

  const handleBtnClick=()=>{
    dispatch(addToCart({ id, title, category, description, image, price, rating,isPrime }));
  }
  ...
  return (
    <div className='flex flex-col p-5 m-5 bg-white relative '>
      <span className=' absolute top-1 right-3 font-xs italic text-gray-500'>{category}</span>
      <div className='h-64  flex flex-col items-center my-3'>
        <Image
          alt={title} 
          className='mx-auto'
          src={image} 
      ...
      <div className='flex mb-3'>
        ...
        {
          isPrime&&<p className='flex items-center mb-3'>
      ...
      <button onClick={handleBtnClick} className='button text-lg w-full mt-auto'>Add to Basket</button>
